...
specTable <- function(atable) {
  ...
  year_arts <- atable %>% 
              pull(yearsRel) %>%
              unique()
  if (str_detect(repertoire_ids, "CFAR")) {
    gpath <- paste(repertoire_ids, "tsv", sep = ".")  
  } else {
    gpath <- paste(paste(repertoire_ids, "convergence", "groups", sep = "-"), "txt", sep = ".")
  }
...
supression_table <- vl_table %>% 
                    ...
                    mutate(supression_percent = `TRUE` * 100/(`FALSE` + `TRUE`),
                           n = `FALSE` + `TRUE`,
                           supression = case_when(supression_percent > 50 & supression_percent <= 100 ~ "Median VL < 200 copies/mL Post-ART",
                                                         supression_percent <= 50 ~ "Median VL >= 200 copies/mL Post-ART"))
cc_table <- cc_table %>% 
...
gini_table  <-  gtable %>% 
                mutate(timePoint = case_when(yearsRel < 0 ~ "Pre-ART", 
                                             yearsRel >= 0 ~ "Post-ART",
                                             is.na(yearsRel) ~ "Normal")) %>% 
                group_by(repertoire_id) %>% 
                group_split() %>% 
                purrr::map(getGini) %>% 
                bind_rows() 
...
itable <- itable %>% 
          ...
          mutate(supression = replace_na(supression, "Normal"),
...
cc_2class_plot <- ggboxplot(cc_table, 
                            ...
                            y = "result",
                            color = "time_point", 
                            palette = c("Pre-ART" = "#7570b3", "Post-ART" = "#d95f02", "Normal" = "#1b9e77"),
                            add = "jitter", 
                            notch = TRUE, 
...
getIndTest <- function(itable, cc_table) {
  high_cctable <- cc_table %>% 
                 filter(supression == "High") %>% 
                 select(time_point, result) %>% 
                 mutate(time = if_else(time_point == "Post-ART", 1, 0)) %>% 
                 select(time, result)
  low_cctable <- cc_table %>% 
                 filter(supression == "Low") %>% 
                 select(time_point, result) %>% 
                 mutate(time = if_else(time_point == "Post-ART", 1, 0)) %>% 
                 select(time, result)
  low_itable <- itable %>% 
                filter(supression == "Low" & time_point != "Normal") %>% 
                select(time_point, rarefied_count, cluster_gini, vertex_gini) %>% 
                mutate(time = if_else(time_point == "Post-ART", 1, 0)) %>% 
                select(time, rarefied_count, cluster_gini, vertex_gini)
  high_itable <- itable %>% 
                filter(supression == "High" & time_point != "Normal") %>% 
                select(time_point, rarefied_count, cluster_gini, vertex_gini) %>% 
                mutate(time = if_else(time_point == "Post-ART", 1, 0)) %>% 
                select(time, rarefied_count, cluster_gini, vertex_gini)
  low_src <- pvalue(independence_test(time ~ rarefied_count, data = low_itable))
  low_svg <- pvalue(independence_test(time ~ vertex_gini, data = low_itable))
  low_scg <- pvalue(independence_test(time ~ cluster_gini, data = low_itable))
  low_scc <- pvalue(independence_test(time ~ result, data = low_cctable))
  high_src <- pvalue(independence_test(time ~ rarefied_count, data = high_itable))
  high_svg <- pvalue(independence_test(time ~ vertex_gini, data = high_itable))
  high_scg <- pvalue(independence_test(time ~ cluster_gini, data = high_itable))
  high_scc <- pvalue(independence_test(time ~ result, data = high_cctable))

  ind_itable <- itable %>% 
                filter(time_point != "Normal") %>% 
  ...
  test_tibble <- tibble(group = c("Low: Post-ART Vs Pre-ART", "Low: Post-ART Vs Pre-ART", "Low: Post-ART Vs Pre-ART", "Low: Post-ART Vs Pre-ART", "High: Post-ART Vs Pre-ART", "High: Post-ART Vs Pre-ART", "High: Post-ART Vs Pre-ART", "High: Post-ART Vs Pre-ART", "Pre-ART Vs Post-ART", "Pre-ART Vs Post-ART", "Pre-ART Vs Post-ART", "Pre-ART Vs Post-ART"),
                        ...
                        pvalue = c(low_src, low_svg, low_scg, low_scc, high_src, high_svg, high_scg, high_scc, ind_rc, ind_vg, ind_cg, ind_cc)) 
