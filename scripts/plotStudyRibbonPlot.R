...
plotSankey <- function(sample_table, vtable, cdtable, dtable, cltable, pbfactor) {
    set.seed(12345)
    # Filter samples
    ptid <- sample_table %>% pull(patientID) %>% unique()
    vtable <- vtable %>%
...
sankey_plot <- atable %>% 
               ...
               group_by(patientID) %>% 
               group_split() %>% 
               purrr::map(~plotSankey(.x, vl_table, cc_table, infection_table, itable, pbfactor)) %>% 
               wrap_plots(nrow = 6, ncol = 5, heights = 2, widths = 3)
out_path <- "CFAR_topFreq.pdf"
ggsave(out_path, sankey_plot, width = 15, height = 12, device = "pdf", units = "in")
